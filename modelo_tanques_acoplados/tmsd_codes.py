...
def prbs_signal(N, b, m):
    ...
    @Author MATLAB: Luis A. Aguirre - BH 18/10/95
        - revision 01/02/1999
    @Maintainer PYTHON: Fernando Souza, on Feb, 06, 2021
    @Colaboration: Felipe F. Rocha
    :param N: Size of signal
    :param b: number bits
    :param m: sequence n value
    :return: signal PRBS
    """
    
    u = np.zeros(N);
    x = (( np.random.randint(2**b) & (1 << np.arange(b))) > 0).astype(int)
...
def myccf(y,u,lag,flag1=0,flag2=1,cor_stem ='green',cor_conf='blue',title="",legend = "empty", fig_size = (12, 4)):
